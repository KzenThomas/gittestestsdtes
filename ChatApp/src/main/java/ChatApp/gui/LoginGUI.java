...
public class LoginGUI extends JFrame {
		...
		loginButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if (e.getSource() == loginButton) {
					...
					if(isPasswordDuplicate(username)) {
                        JOptionPane.showMessageDialog(null, "user already exists!", "USERNAME ERROR", JOptionPane.ERROR_MESSAGE);
                    }
	...
	private void preloadPasswordsFromDB() {
		Iterable<Login> logins = loginRepo.findAll();
		List<String> user = new ArrayList<>();
		logins.forEach(e -> user.add(e.getwachtwoord()));
		for(String s : user) {
			duplicates.put(s, 1);
		}
	}
	
	private boolean isPasswordDuplicate(String user) {
